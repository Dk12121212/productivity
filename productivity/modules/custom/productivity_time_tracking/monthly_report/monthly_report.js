...
(function ($) {
  ...
  function create_new_url(base_url, all, year) {
    ...
    if (year) {
      ...
      return base_url + "/monthly-report-old/" + project_id + "/" + res[1].trim() + "/all/" + estimated;
    }

    // All time link.
    if (all) {
      return base_url + "/monthly-report-old/" + project_id + "/all/all/" + estimated;
    }

    // Specific month link
    return base_url + "/monthly-report-old/" + project_id + "/" + res[1].trim() + "/" + get_month_num(res[0].trim()) + "/" + estimated;
  }
